...
func (rq *RoomQuery) QueryBookings() *ProfileQuery {
...
func (rq *RoomQuery) QueryAvailability() *AvailableRoomQuery {
	query := (&AvailableRoomClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) QueryBookingsData() *BookingQuery {
...
func (rq *RoomQuery) WithBookings(opts ...func(*ProfileQuery)) *RoomQuery {
	query := (&ProfileClient{config: rq.config}).Query()
...
func (rq *RoomQuery) WithAvailability(opts ...func(*AvailableRoomQuery)) *RoomQuery {
	query := (&AvailableRoomClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoomQuery) WithBookingsData(opts ...func(*BookingQuery)) *RoomQuery {
	query := (&BookingClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withBookingsData = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withBookings; query != nil {
		if err := rq.loadBookings(ctx, query, nodes,
			func(n *Room) { n.Edges.Bookings = []*Profile{} },
			func(n *Room, e *Profile) { n.Edges.Bookings = append(n.Edges.Bookings, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withAvailability; query != nil {
	...
	if query := rq.withBookingsData; query != nil {
		if err := rq.loadBookingsData(ctx, query, nodes,
			func(n *Room) { n.Edges.BookingsData = []*Booking{} },
			func(n *Room, e *Booking) { n.Edges.BookingsData = append(n.Edges.BookingsData, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadBookings(ctx context.Context, query *ProfileQuery, nodes []*Room, init func(*Room), assign func(*Room, *Profile)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(room.BookingsTable)
		s.Join(joinT).On(s.C(profile.FieldID), joinT.C(room.BookingsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(room.BookingsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(room.BookingsPrimaryKey[1]))
...
func (rq *RoomQuery) loadAvailability(ctx context.Context, query *AvailableRoomQuery, nodes []*Room, init func(*Room), assign func(*Room, *AvailableRoom)) error {
	...
	query.withFKs = true
	query.Where(predicate.AvailableRoom(func(s *sql.Selector) {
		s.Where(sql.InValues(room.AvailabilityColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.room_availability
		if fk == nil {
			return fmt.Errorf(`foreign-key "room_availability" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (rq *RoomQuery) loadBookingsData(ctx context.Context, query *BookingQuery, nodes []*Room, init func(*Room), assign func(*Room, *Booking)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Room)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Booking(func(s *sql.Selector) {
		s.Where(sql.InValues(room.BookingsDataColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "room_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
