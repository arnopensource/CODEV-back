...
func (rc *RoomCreate) SetBuilding(s string) *RoomCreate {
	rc.mutation.SetBuilding(s)
	return rc
}
...
func (rc *RoomCreate) AddBookings(p ...*Profile) *RoomCreate {
...
func (rc *RoomCreate) AddAvailabilityIDs(ids ...int) *RoomCreate {
	rc.mutation.AddAvailabilityIDs(ids...)
	return rc
}

// AddAvailability adds the "availability" edges to the AvailableRoom entity.
func (rc *RoomCreate) AddAvailability(a ...*AvailableRoom) *RoomCreate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
func (rc *RoomCreate) check() error {
	...
	if _, ok := rc.mutation.Building(); !ok {
		return &ValidationError{Name: "building", err: errors.New(`ent: missing required field "Room.building"`)}
	}
...
func (rc *RoomCreate) createSpec() (*Room, *sqlgraph.CreateSpec) {
	...
	if nodes := rc.mutation.AvailabilityIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
